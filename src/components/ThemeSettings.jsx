          ...
          <div className='flex gap-3'>
            {themeColors.map((item, index) => (
              <TooltipComponent key={index} content={item.name} position='TopCenter'>
                <div className='relative cursor-pointer items-center flex gap-5 mt-2'>
                  <button type='button' className='rounded-full h-10 w-10 cursor-pointer' style={{ backgroundColor: item.color }} onClick={() => setColor(item.color)}>
